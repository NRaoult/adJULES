...
program prgtstvtlm
  ...
  integer :: nbeg              ! first component to test
  integer :: nend             ! last component to test
  ...
  real    :: adspeed = 7.         ! minimum relative speed of TLM
  character*(7) :: cfile = 'tst.par'
  integer :: i, ncol
  namelist /check/ nbeg, nend, nstep, neps, eps, epsstep, epsmach, acc, niter, tlspeed, adspeed
...
!  read (UNIT=87, nml=check, END=100, ERR=100 )
  read (UNIT=87, nml=check)
  print '(A)',' parameters for vtl-check read from file ', cfile
  write (*,nml=check)
100 close(UNIT=87)
  ...
  do i = nbeg,nend,nstep
     ncol = ncol+1
  enddo
  print *,'number of vectors for vtlm:',ncol
...
subroutine doit( n, m, ncol, nbeg, nend, nstep, neps, epsbeg, epsstep, epsmach, acc, niter, tlspeed )
  use mo_timing
  use inout, ONLY : nts, weight , xoff ! Jupp
  ...
  real    :: d_y(ncol,m)
  real    :: gfd(ncol,m)
  real    :: y1(m), y2(m), ytlm(m), xmemo, rel(ncol,m), deltay(m), absmax
 ...
 ! initialise the model and set control variables
  ...
  do iter=1 , niter 
     ...
     call func_tl( ncol, n, x, d_x, ncol, m, ytlm, d_y, ncol) 
